...
int main() {
	...
	arena.execute([]{
		...
		auto [data, weights, beta, groundTruth] = generate_data<FPType>(meta, "data.dat");
		...
		auto [data_extra, weights_extra, beta_extra, groundTruth_extra] = generate_data<FPType>(meta, "extra.dat");
		std::cout << "Extra" << std::endl;
		for (auto& value : data_extra) {
			std::cout << value << ' ';
		}
		std::cout << std::endl;
		constexpr size_t real_runs = 10;
		...
		for (size_t run_index = 0; run_index < real_runs; ++run_index) {
			// verbose_print(verbosity, "# Start no optimal solution: forward");
			auto result_forward_noopt = logreg_noopt::forward<FPType>(meta, data, weights, groundTruth, beta, verbosity);
			// verbose_print(verbosity, "# Start no optimal solution: backward");
			auto result_gradient_noopt = logreg_noopt::gradient<FPType>(meta, data, weights, groundTruth, beta, result_forward_noopt, verbosity);
			
		}
		__itt_task_end(domain);
		auto finish_noopt = std::chrono::system_clock::now();
		verbose_print(verbosity, "# No optimal solution finished");
		std::cout << "No opt time (sec): " << std::chrono::duration_cast<std::chrono::microseconds>(finish_noopt - start_noopt).count() / 1e6 / real_runs << std::endl;

		auto result_forward_noopt = logreg_noopt::forward<FPType>(meta, data, weights, groundTruth, beta, verbosity);
		auto result_gradient_noopt = logreg_noopt::gradient<FPType>(meta, data, weights, groundTruth, beta, result_forward_noopt, verbosity);

		constexpr size_t extra_runs = 7;
		for (size_t run_index = 0; run_index < extra_runs; ++run_index) {
			auto result_forward_extra = logreg_noopt::forward<FPType>(meta, data_extra, weights_extra, groundTruth_extra, beta_extra, false);
			logreg_noopt::gradient<FPType>(meta, data_extra, weights_extra, groundTruth_extra, beta_extra, result_forward_extra, false);
		}
		...
		for (size_t run_index = 0; run_index < real_runs; ++run_index) {
			auto [result_forward_opt, result_gradient_opt] = logreg_opt::forward_and_gradient<FPType>(meta, data, weights, groundTruth, beta, verbosity);		
		}
		__itt_task_end(domain);
		verbose_print(verbosity, "# Optimal solution finished");
		auto finish_opt = std::chrono::system_clock::now();
		std::cout << "Opt time (sec): " << std::chrono::duration_cast<std::chrono::microseconds>(finish_opt - start_opt).count() / 1e6 / real_runs << std::endl;

		auto [result_forward_opt, result_gradient_opt] = logreg_opt::forward_and_gradient<FPType>(meta, data, weights, groundTruth, beta, verbosity);		

		bool forward_equality = metrics::check_forward_equality(result_forward_noopt, result_forward_opt);
		if (!forward_equality) {
			verbose_print(true, "!!! Forward results are not equal");
		} else {
			verbose_print(true, "# Forward results are equal");
		}
		bool gradient_equality = metrics::check_gradient_equality(result_gradient_noopt, result_gradient_opt);
